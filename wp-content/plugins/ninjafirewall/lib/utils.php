...
function nfw_garbage_collector() {
	...
	if (! empty( $nfw_options['widgetnews'] ) ) {
		$maxnews = (int) $nfw_options['widgetnews'];
		nfw_fetch_secnews( $maxnews, 0 );
	}
}

// ---------------------------------------------------------------------
// Fetch the widget's news.

function nfw_fetch_secnews( $maxnews, $return = 1 ) {

	// Fetch the last $maxnews posts (except changelog):
	$url = 'https://blog.nintechnet.com/wp-json/wp/v2/posts?per_page='.
				$maxnews .'&categories_exclude=37';
